...
int es10b_load_bound_profile_package(struct euicc_ctx *ctx, struct es10b_load_bound_profile_package_result *result)
{
    ...
    printf("%s", ctx->http._internal.b64_bound_profile_package);

    ctx->apdu.interface->logic_channel_close(ctx, ctx->apdu._internal.logic_channel);

    ctx->http.server_address = "127.0.0.1:8000";
